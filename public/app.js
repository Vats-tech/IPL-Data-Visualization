...
function visualizeData(data){
    ...
    const obj=constructingDataForMatchesWonByEachTeam(data.matchesWonByEachTeam);
    const wonData=[];
    for(i in obj){
        (w={}).name=i,
        w.data=obj[i],
    ...
    const obj1=constructingDataForWonMatchPerVenue(data.matchesWonByEachTeamPerVenue);
    const matchesPerVenue=[];
    for(i in obj1){
        (w={}).name=i,
        w.data=obj1[i],
        matchesPerVenue.push(w);
    }
    visulizeMatchesWonByEachTeamPerVenue(matchesPerVenue,data.matchesWonByEachTeamPerVenue);
    return;
}
...
function constructingDataForMatchesWonByEachTeam(matchesWonByEachTeam){
    ...
    for(let i in matchesWonByEachTeam){
        for(let team in  matchesWonByEachTeam[i]){
            ...
            obj[team].push(matchesWonByEachTeam[i][team]);
            teamName[team]=false;
        }
...
function visualizeBowlersEconomy(bowlersData){
    ...
    for(let data in bowlersData){
        let economy = ((bowlersData[data].totalRuns)*6)/bowlersData[data].noOfBall;
...
function constructingDataForWonMatchPerVenue(matches){
    ...
    for(let venueName of venue){
        for(let t of Object.keys(matches[venueName]))
        helper.add(t);
    }
    teamName=helper;
